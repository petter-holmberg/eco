...
inline constexpr default_array_growth_algorithm default_array_growth{};

inline malloc_allocator default_array_alloc{};
...
public:
  ...
  operator=(extent const& x) -> extent&
  {
    ...
    if (capacity() < x.size()) { // Strong exception guarantee
      [[maybe_unused]] invariant: assert(*this != x);
      auto temp{x};
      swap(temp);
  ...
  adjust_unused_capacity(Size n)
  {
    ...
    [[maybe_unused]] post: assert(unused_capacity() == n);
  }
  ...
  push_back(Args &&...args) -> T&
  {
    ...
    [[maybe_unused]] invariant: assert(unused_capacity() > 0);
  ...
  insert_space(T* insertion_point, Size n_elements, Writer writer) -> T*
  {
    ...
    [[maybe_unused]] invariant: assert(start);
  ...
  {
    [[maybe_unused]] pre: assert(n_elements <= size());
    if (n_elements > 0) {
